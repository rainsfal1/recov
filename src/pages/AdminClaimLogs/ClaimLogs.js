...
export default function ClaimLogs() {
    ...
    const getClaimData = async (page, selectedOption) => {
        try {
            ...
            const response = await fetch(`http://localhost:3000/api/v1/claim/getClaim?page=${page}&sort=${selectedOption}` // Use selectedOption in API call
            );
    ...
    return (_jsxs("div", { className: "container mx-auto px-4  md:px-6 md:py-12", children: [_jsx(LogsHeader, { title: "Claim Logs", placeholder: "Search Claim logs...", baseRoute: "/home/claim-logs", selectedOption: selectedOption, setSelectedOption: setSelectedOption, options: options }), _jsx(Table, { data: claimData }), error && _jsx("p", { className: "text-red-500", children: error }), _jsx(Pagination, { currentPage: currentPage, totalPages: totalPages, onPageChange: (newPage) => {
